...
// var debug = require('debug')('sslayer:ScreenShotLayerHttpClient');
 ...
 *  the instance's path (e.g. 'captureScreenshot')
  ...
  sslayerMethod: function (spec) {
    ...
    return function () {
      ...
      return this.wrapTimeout(new Promise((function (resolve, reject) {
        for (var i = 0, l = urlParams.length; i < l; ++i) {
          ...
          if (!arg) {
            ...
            return reject(new Error(
              'sslayer: Argument "' + urlParams[i] + '" required, but got: ' + arg +
        ...
        if (args.length) {
          var err = new Error('sslayer: Unknown arguments (' + args + ').');
          return reject(err);
        }
